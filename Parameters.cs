...
public sealed class Parameters: IEnumerable<Parameter>
{
    ...
    public Parameters(params Parameter[] parameters)
    {
        ...
        parameterMap.Add(paramHelp.LongNameLowered, paramHelp);
        parameterFlagMap.Add(paramHelp.Flag, paramHelp);
    ...
    public ParamsState Evaluate(string[] args)
    {
        ...
        foreach (var param in parameters)
        {
            if (param.IsMandatory && !param.IsSet)
            {
    ...
    public void PrintOutParamDescription()
    {
    ...
    private IEnumerator<Parameter> EvalGetParametersEnumerator()
    {
        foreach (var param in parameters)
        {
            while (!param.IsSet && param.IsPositional)
            {
...
public abstract class Parameter
{
    ...
    public Parameter(char flag, string longName, string description, bool isMandatory, bool isPositional) 
    {
...
public sealed class Parameter<T> : Parameter
{
    ...
    public Parameter(char flag, string longName, string description, bool isMandatory = false, bool isPositional = true, T? defaultValue = default, Func<T, bool>? constrain = null): base(flag, longName, description, isMandatory, isPositional)
    {
        this.Value = defaultValue;
        this.HasDefaultValue = defaultValue != null;
        this.constrain = constrain ?? ParameterConstrains.AlwaysTrue;
    }
...
{
    public SwitchParameter(char flag, string longName, string description) : base(flag, longName, description, false, false) { }
...
public static class ParameterConstrains
{
    public static bool AlwaysTrue<T>(T? val) => true;
    public static bool NotNull<T>(T? val) => val != null;
}
